...
public class Znaki {
    ...
    static void izpisi16bit(short[] nizZnakov) {
        for (int i = 0; i < 4; i++) {
            ...
            for (int ind = 0; ind < nizZnakov.length; ind++) {
                for (int j = 0; j < 4; j++) {
                    int maska = 1 << (15 - (i * 4 + j));

                    if ((nizZnakov[ind] & maska) != 0) {
                        System.out.print(crnaPika);
                    } else {
                        System.out.print(belaPika);
                    }
    ...
    static void izpisi16bit(String niz) {
        ...
        for (int i = 0; i < niz.length(); i++) {
            for (char x : abeceda) {
                if (Character.toUpperCase(niz.charAt(i)) == x) {
                    tab[i] = kodeZnakov16bit[count - 1];
                    break;
                } else if (x == ' ') {
                    tab[i] = kodeZnakov16bit[abeceda.length];
                }
                count++;
            }
            count = 1;
    ...
    static void izpisi64bit(String niz) {
        ...
        for (int i = 0; i < niz.length(); i++) {
            for (char x : abeceda) {
                if (Character.toUpperCase(niz.charAt(i)) == x) {
                    tab[i] = kodeZnakov64bit[count];
                    break;
                } else if (x == ' ') {
                    tab[i] = kodeZnakov64bit[abeceda.length];
                }
                count++;
            }
            count = 0;
    ...
    public static void main(String[] args) {
        ...
        // izpisi16bit((short)0b1111100111111001); //A test
        // izpisi16bit((short)0b1111100111111000); //P test
        // izpisi16bit((short)0b1110001001001111); //2 test

        // izpisi16bit(new short[] {(short)0b1111100011101000,
        // (short)0b1111100111111010, (short)0b1111010001001111});
