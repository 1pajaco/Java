...
public class DN09Test5 {
    ...
    public void setUp() {
        ...
        testLinije[2].dodajPostajo(testPostaje[2]);
    ...
    @Test
    public void casiPrihodov() throws FileNotFoundException {
        ByteArrayOutputStream outContent = new ByteArrayOutputStream();
        System.setOut(new PrintStream(outContent));
        DN09.casiPrihodov(3, 5);
        assertEquals("Postaja C:\n" +
                "Linija 1 Avtobus 101 -  0\n" +
                "Linija 2 Avtobus 102 -  2\n" +
                "Linija 3 Avtobus 103 -  1".strip().replace("\r", ""), outContent.toString().strip().replace("\r", ""));
        System.setOut(originalOut);

    }
    @Test
    public void casiPrihodov2() throws FileNotFoundException {
        ByteArrayOutputStream outContent = new ByteArrayOutputStream();
        System.setOut(new PrintStream(outContent));
        DN09.casiPrihodov(8, 5);
        assertEquals("Postaja H:\n" +
                "Linija 3 Avtobus 103 -  0".strip().replace("\r", ""), outContent.toString().strip().replace("\r", ""));
        System.setOut(originalOut);
